...
export function coverageForSignal(
  signalId: string,
  model: ListViewModel,
): CoverageCounts {
  const row = model.objectives.find((entry) =>
    entry.signalIds.includes(signalId),
  );
  if (!row) return {};
  return {
    objective: 1,
...
export function coverageForListItem(
  item: ListViewItem,
  model: ListViewModel,
): CoverageCounts {
  switch (item.type as ObjectType) {
    case "threat":
      return coverageForThreat(item.id, model);
    case "objective": {
      const row = model.objectives.find(
        (entry) => entry.objective.id === item.id,
      );
      return row ? coverageForObjective(row) : {};
    }
    case "signal":
      return coverageForSignal(item.id, model);
    case "rule":
      return coverageForRule(item.id, model);
    default:
      return {};
  }
}
